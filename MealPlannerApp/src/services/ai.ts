...
class AIService {
  ...
  async suggestMealSwap(
    ...
    try {
      ...
      const userPrompt = this.buildMealSwapPrompt(originalMeal, userProfile, excludeRecipes);
...
Always respond with properly formatted JSON that can be parsed by JavaScript's JSON.parse().
  ...
  private buildMealPlanPrompt(request: MealPlanRequest): string {
    const { userProfile, excludeRecipes = [], pantryItems = [], preferredCuisines = [] } = request;
...
Planning Guidelines:
1. Create meals that respect all dietary restrictions
2. Stay within the weekly budget of $${userProfile.weeklyBudget.toFixed(2)}
3. Weekday meals should take no more than ${userProfile.cookingTimePreference.weekday} minutes to prepare
4. Weekend meals can be more elaborate (up to ${userProfile.cookingTimePreference.weekend} minutes)
5. Include a variety of cuisines with emphasis on the user's preferences
6. Create a balanced plan with appropriate portion sizes
7. Reuse ingredients across meals when possible to reduce waste
8. Adjust complexity based on the user's cooking skill level
