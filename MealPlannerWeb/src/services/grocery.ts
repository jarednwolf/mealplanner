...
class GroceryService {
  ...
  async generateGroceryList(
    ...
    try {
      ...
      // Step 3: Convert to grocery items with real pricing
      const groceryItems = await this.createGroceryItemsWithRealPricing(filteredIngredients, pantryItems);
  ...
  private aggregateIngredients(mealPlan: MealPlan): Map<string, AggregatedIngredient> {
    ...
    mealPlan.meals.forEach(meal => {
      // Check if ingredients exist before iterating
      if (!meal.ingredients || !Array.isArray(meal.ingredients)) {
        console.warn(`Meal ${meal.recipeName} has no ingredients`);
        return;
      }
  ...
  /**
   * Convert aggregated ingredients to grocery items with real pricing
   */
  private async createGroceryItemsWithRealPricing(
    filteredIngredients: Map<string, AggregatedIngredient>,
    pantryItems: PantryItem[]
  ): Promise<GroceryItem[]> {
    ...
    // Process each ingredient with real pricing
    for (const [key, ingredient] of filteredIngredients) {
      const pantryItem = pantryItems.find(
        item => this.normalizeIngredientName(item.name) === this.normalizeIngredientName(ingredient.name)
      );
      
      // Get real price for the ingredient
      let realPrice = ingredient.totalPrice; // Default to estimated price
      try {
        realPrice = await pricingService.getBestPrice(
          ingredient.name,
        ...
        console.error(`Failed to get real price for ${ingredient.name}, using estimate`);
      }
