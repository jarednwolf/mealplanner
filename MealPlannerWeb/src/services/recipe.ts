...
class RecipeService {
  ...
  async searchRecipes(params: RecipeSearchParams): Promise<Recipe[]> {
    ...
    // Use direct API if key is available and enabled
    if (config.useRealRecipes && config.spoonacularApiKey) {
      try {
        const apiParams = {
          query: params.query,
        ...
        const recipes = await recipeApiService.searchRecipes(apiParams);
        ...
        console.error('Direct API search failed, falling back to proxy:', error);
      }
    }

    // Use Firebase proxy as fallback or primary method
    try {
  ...
  async getRecipeById(id: string): Promise<Recipe> {
    ...
    if (config.useRealRecipes && config.spoonacularApiKey) {
      try {
        ...
        console.error('Direct API fetch failed, falling back to proxy:', error);
      }
    }
  ...
  async getIngredientPrices(ingredients: string[]): Promise<Record<string, number>> {
    ...
    await Promise.all(
      ingredients.map(async (ingredient) => {
        try {
          // Get best price for the ingredient
          // Default to 1 unit for now - could be improved with better parsing
          const price = await pricingService.getBestPrice(ingredient, 1, 'each', zipCode);
          prices[ingredient] = price;
        } catch (error) {
          console.error(`Error getting price for ${ingredient}:`, error);
          // Fallback to a reasonable default
          prices[ingredient] = 2.50;
        }
      })
    );
